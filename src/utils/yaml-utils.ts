...
export function isScalar(
  node: yamlAstParser.YAMLNode,
): node is yamlAstParser.YAMLScalar {
...
export function getValueByKey(
  mapNode: yamlAstParser.YamlMap,
  key: string,
): yamlAstParser.YAMLNode | undefined {
...
export function isPositionInNode(
  ...
  if (
    position.line === nodeStart.line &&
    position.character < nodeStart.character
  ) {
    return false;
  }

  if (
    position.line === nodeEnd.line &&
    position.character > nodeEnd.character
  ) {
    return false;
  }
...
export function findPackageAtPosition(
  ...
  const currentSection = dependencySections.find(
    (section) => section.node && isPositionInNode(position, section.node, text),
  );
